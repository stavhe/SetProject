...
public class Table {
     ...
     * Mapping between a card and the slot it is in (null if none).
     */
    protected final LinkedList<Integer>[] slotToToken; // ids of players thats have token on each slot (if any)
    protected Semaphore[] locks;
    ...
    public Table(Env env, Integer[] slotToCard, Integer[] cardToSlot) {
        ...
        slotToToken = new LinkedList[env.config.tableSize];
        locks = new Semaphore[env.config.tableSize];
        for (int i = 0; i < env.config.tableSize; i++) {
            slotToToken[i] = new LinkedList<>();
    ...
    public Integer getSlotToCard(int i) {
        return slotToCard[i];
    }
}